...
int main() {
    ...
    if (n <= 20) {
        ...
        int result_bf = knapsackBruteForce(items, W);
        auto end_bf = std::chrono::high_resolution_clock::now();
        std::cout << "Brute force: " << knapsackBruteForce(items, W) << std::endl;
    ...
    int result_dp = knapsackDynamicRollingArray(items, W);
    ...
    while (result_ga != result_dp) {
        ...
        // result_ga = knapsackGeneticAlgorithm_o(items, W, populationSize, generations, crossoverRate, mutationRate);
